...
const JasonDashboardPage: React.FC = () => {
    ...
    const validateSunoUrl = (url: string): boolean => {
        if (!url) {
            setSunoUrlError('');
            return true;
        }
        try {
            ...
            if (!parsedUrl.hostname.includes('suno.com')) {
                setSunoUrlError('URL must be a valid Suno playlist URL (suno.com).');
                return false;
            }
            setSunoUrlError('');
            return true;
        } catch (_) {
            setSunoUrlError('Please enter a valid URL format.');
            return false;
        }
    };

    const handleSunoUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleGenerateArtistPage = async () => {
        if (!sunoUrl.trim() || sunoUrlError) {
            toast.error("Please enter a valid Suno playlist URL.");
            return;
        }
                    ...
                    <div>
                        <div className="flex items-center gap-4">
                            <input
                                type="url"
                                value={sunoUrl}
                                onChange={handleSunoUrlChange}
                                placeholder="https://suno.com/playlist/..."
                                className={`w-full bg-brand-bg rounded-lg p-3 text-white placeholder-brand-text-secondary focus:outline-none focus:ring-2 ${sunoUrlError ? 'ring-red-500' : 'focus:ring-brand-primary'}`}
                                disabled={isPageGenerating}
                            />
                            <button
                                onClick={handleGenerateArtistPage}
                                disabled={isPageGenerating || !sunoUrl || !!sunoUrlError}
                        ...
                        {sunoUrlError && <p className="text-red-400 text-sm mt-2">{sunoUrlError}</p>}
                    </div>
                </Section>
