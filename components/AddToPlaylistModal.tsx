...
const AddToPlaylistModal: React.FC<AddToPlaylistModalProps> = ({ trackId, onClose }) => {
    ...
    const handleCreateAndAdd = (e: React.FormEvent) => {
        ...
        createUserPlaylist(newPlaylistTitle, '');
        // The context update will cause a re-render. We can just reset the form state.
        // This UX is better as the modal stays open, showing the new playlist.
                ...
                {showCreateForm ? (
                    <form onSubmit={handleCreateAndAdd} className="space-y-2 pt-4 border-t border-brand-surface">
                        <input
                            ...
                            autoFocus
                        />
                        <button type="submit" className="w-full bg-brand-primary py-2 rounded-lg font-semibold">Create Playlist</button>
                    </form>
                ) : (
