...
import { Artist, NewsArticle, DisplayTrack, GalleryImage, PageID, StudioSubmission, SubmissionStatus, ConstellationItem, Release, Track, SpotlightItem, Playlist, CurrentlyPlayingTrack, Asset, TeoApp, FriendArtist, PlaylistCategory, SmtVideo, StudioActionCosts, ApiKeys, FooterContent, SpecializedAgent, JasonChatMessage, AgentState } from '../types';
import { 
...
interface ContentContextType extends ContentState {
  ...
  toggleLiveMode: (isActive: boolean) => void;
  updateAgentState: (newState: Partial<AgentState>) => void;
}
    ...
    try {
        ...
        if (storedContent) {
            ...
            if (typeof parsed === 'object' && parsed !== null) {
                // Merge parsed data with defaults to ensure all keys exist (handling updates/migrations)
                const merged = { ...defaults, ...parsed };
                
                // Post-merge validation: Ensure critical properties that should be arrays are arrays.
                if (!Array.isArray(merged.artists)) merged.artists = defaults.artists;
                if (!Array.isArray(merged.playlists)) merged.playlists = defaults.playlists;
                if (!Array.isArray(merged.trendingTrackIds)) merged.trendingTrackIds = defaults.trendingTrackIds;
                if (!Array.isArray(merged.smtVideos)) merged.smtVideos = defaults.smtVideos;
                
                // Ensure nested objects exist
                if (!merged.studioCosts) merged.studioCosts = defaults.studioCosts;
...
export const ContentProvider: React.FC<ContentProviderProps> = ({ children }) => {
  const [state, setState] = useState<ContentState>({
      ...
      agentState: {
          ...
          parameters: { tempo: 100, complexity: 0, instruments: [] }
      }
  });
  ...
  useEffect(() => {
    try {
        const { galleryImages, studioSubmissions, specializedAgentChats: _, isLiveMode, agentState, ...persistedState } = state;
  ...
  const playTrack = (track: Track, release: Release, artist: Artist) => {
      // If Live Mode is active, turn it off to play a regular track
      if (state.isLiveMode) {
  ...
  const toggleLiveMode = (isActive: boolean) => {
      if (isActive) {
          clearCurrentTrack(); // Stop any current playback
      }
      setState(prev => ({ ...prev, isLiveMode: isActive }));
  };

  const updateAgentState = (newState: Partial<AgentState>) => {
      setState(prev => ({
          ...prev,
          agentState: { ...prev.agentState, ...newState }
      }));
  };
